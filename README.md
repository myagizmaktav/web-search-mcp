...
You can also run this MCP server using Docker for easier deployment and isolation.
...
- Playwright browsers pre-installed for content extraction
- Non-root user for security
- Proper stdin/stdout handling for MCP communication

**Note:** The container runs the MCP server and communicates via stdin/stdout, so it's designed to work with MCP clients that can connect to containerized servers.
